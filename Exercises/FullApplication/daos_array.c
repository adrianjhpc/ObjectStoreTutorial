...
int main(int argc, char *argv[]) {
  ...
  // This example should be very similar to the daos_individual.c example/implementation, but only creates a single array for all processes rather than an array per process
  ...
  // Create the name for the data object to be written,this will be the same name for all the processes.
  // You should update the name below from tuXXX to your username
  snprintf(objname, 100, "tuXXX-data");
  ...
  // Once you have populated the daos_array_create above you can uncomment the functionaliy below which handles the condition where
  // you have already created the array and just want to open it.
  //if (ierr == -1004) {
  ...
  for (i = 1; i < (ny+1)*ng; i++) {
    ...
    // TODO You will need to update the setting below to write into the correct position in global array
    //iod.arr_nr = ;
    //rg.rg_len = ;
    //rg.rg_idx = ;
    //iod.arr_rgs = &rg;
    //sgl.sg_nr = ;
    //d_iov_set(&iov, NEED TO ADD ARGUMENTS);
    //sgl.sg_iovs = &iov;
    ...
    //ierr = daos_array_write(NEED TO ADD ARGUMENTS);
  }
  ...
  //ierr = daos_array_close(NEED TO ADD ARGUMENTS);
