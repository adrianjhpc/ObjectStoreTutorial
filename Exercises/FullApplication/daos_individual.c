...
int main(int argc, char *argv[]) {
  ...
  // We have done the pool and container create and connect for you to simplify this process
  if(mpi_rank == 0){
  ...
  // You can change tuXXX to your username
  snprintf(objname, 100, "tuXXX-process-rank-%d", mpi_rank);
  ...
  // the object class needs to be hard coded to OC_EC_2P1G8 at the moment to match the container we are using
  // The hints and args can both be set to 0
  //daos_array_generate_oid(NEEDS ARGUMENTS ADDED);
  ...
  //ierr = daos_array_create(NEEDS ARGUMENTS ADDED);


  // The code below deals with the case where you've already created the array so it just needs opened not created
  // Once you have completed the daos_array_create function you can just uncomment this one
  if (ierr == -1004) {
  ...
  for (i = 1; i < (ny+1)*ng; i++) {
    ...
    //ierr = daos_array_write(NEEDS ARGUMENTS ADDED);
  }
  ...
  //ierr = daos_array_close(NEEDS ARGUMENTS ADDED);
